...
// التأكد من وجود postId في الرابط
if (!isset($_GET['postId'])) {
    die("لم يتم تحديد المنشور.");
}
$postId = intval($_GET['postId']);

// جلب بيانات المنشور والتأكد من أن المستخدم مالكه
$stmt = $conn->prepare("SELECT * FROM posts WHERE PostID = ? AND UserID = ?");
$stmt->bind_param("ii", $postId, $user_id);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows == 0) {
    die("لا يوجد منشور بهذا المعرف أو ليس لديك صلاحية التعديل عليه.");
}
$post = $result->fetch_assoc();
$stmt->close();

// معالجة عملية التعديل عند إرسال النموذج
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($title == "" || $description == "") {
        ...
        // الاحتفاظ بصورة المنشور الحالية، وإذا تم رفع صورة جديدة يتم تحديثها
...
<head>
    ...
    <title>تعديل الإعلان - كِفَاء</title>
...
<input type="hidden" name="postId" value="<?php echo $post['PostID']; ?>">
            ...
            <div class="form-group">
                ...
                <select id="category" name="category" required>
                    <option value="electronics" <?php if($post['Category'] == "electronics") echo "selected"; ?>>الإلكترونيات</option>
                    <option value="clothing" <?php if($post['Category'] == "clothing") echo "selected"; ?>>الملابس</option>
                    <option value="home" <?php if($post['Category'] == "home") echo "selected"; ?>>الأثاث</option>
                    <option value="books" <?php if($post['Category'] == "books") echo "selected"; ?>>الكتب</option>
                    <option value="services" <?php if($post['Category'] == "services") echo "selected"; ?>>الخدمات</option>
                    <option value="health" <?php if($post['Category'] == "health") echo "selected"; ?>>الصحة</option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <small>يمكنك ترك هذا الحقل فارغًا إذا كنت لا تريد تحديث الصورة.</small>
            </div>
